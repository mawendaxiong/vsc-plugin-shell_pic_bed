...
function getUpload(config) {
    switch (config.base.uploadMethod) {
        ...
        case 'GitHub': return new uploads_1.default.GitHub(config);
        ...
        case 'Cloudflare': return new uploads_1.default.Cloudflare(config);
    }
    return null;
}
...
async function formatName(format, filePath, isPaste) {
    let saveName = format;
    let variables = [
...
function getRichText() {
    return new Promise((resolve, reject) => {
        ...
        if (platform === 'win32') {
            ...
            let result = '';
            let ascript = (0, child_process_1.spawn)('sh', [scriptPath]);
            ascript.on('error', (e) => {
                vscode.window.showErrorMessage(e);
            });
            ascript.on('exit', (code, signal) => {
...
function getTmpFolder() {
    let savePath = path.join((0, os_1.tmpdir)(), pkg.name);
...
function noticeComment(context) {
    ...
    if (!notice && usetimes > 100) {
        confirm(locale['like.extension'], [locale['like.ok'], locale['like.no'], locale['like.later']])
            ...
            switch (option) {
                case locale['like.ok']:
                    (0, child_process_1.exec)(`${getOpenCmd()} https://marketplace.visualstudio.com/items?itemName=hancel.markdown-image`);
